import { ADMIN_ACTION_TTL_MINUTES, CALLBACK_GROUP_DISABLE, CALLBACK_GROUP_ENABLE, CALLBACK_GROUP_LEADERBOARD, CALLBACK_GROUP_LEADERBOARD_MENU, CALLBACK_GROUP_SHOW, CALLBACK_GROUP_SUMMARY, CALLBACK_GROUP_SUMMARY_MENU, CALLBACK_LEADERBOARD_CUSTOM, CALLBACK_LEADERBOARD_MENU, CALLBACK_LEADERBOARD_SET, CALLBACK_LEADERBOARD_TOGGLE, CALLBACK_LEADERBOARD_WINDOW_CUSTOM, CALLBACK_LEADERBOARD_WINDOW_MENU, CALLBACK_LEADERBOARD_WINDOW_SET, CALLBACK_LINUXDO_TOKEN_DELETE, CALLBACK_LINUXDO_TOKEN_MENU, CALLBACK_LINUXDO_TOKEN_SET, CALLBACK_PANEL_LIST, CALLBACK_PANEL_OPEN, CALLBACK_PANEL_SYNC, CALLBACK_SCHEDULE_CUSTOM, CALLBACK_SCHEDULE_MENU, CALLBACK_SCHEDULE_SET, CALLBACK_SPOILER_DELETE, CALLBACK_SPOILER_MENU, CALLBACK_SPOILER_TOGGLE, CALLBACK_LINUXDO_MENU, CALLBACK_LINUXDO_TOGGLE, CALLBACK_SCRAPE_GEO_MENU, CALLBACK_SCRAPE_GEO_SET, CALLBACK_SCRAPE_GEO_CUSTOM, CALLBACK_SCRAPE_SUPER_MENU, CALLBACK_SCRAPE_SUPER_TOGGLE, DEFAULT_LEADERBOARD_WINDOW, DEFAULT_LEADERBOARD_SCHEDULE, DEFAULT_SCHEDULE, KV_SYNC_WINDOW_MS, LEADERBOARD_WINDOW_PRESETS, SCHEDULE_CUSTOM_OPTIONS, SCHEDULE_PRESETS, SCRAPE_GEO_PRESETS, } from "../constants";
import { decodeCallbackValue, encodeCallbackValue, isOwnerUser, parseDuration, truncateLabel, } from "../utils";
import { clearAdminAction, deleteGlobalLinuxdoToken, deleteScrapeGeoCode, ensureSchema, getAdminAction, getAllGroups, getGlobalLinuxdoToken, getScrapeGeoCode, getGroupConfig, insertGroupConfig, openKvSyncWindow, saveGroupMessage, setAdminAction, setGlobalLinuxdoToken, setScrapeGeoCode, getScrapeSuper, setScrapeSuper, updateGroupEnabled, updateGroupLeaderboardEnabled, updateGroupLeaderboardSchedule, updateGroupLeaderboardWindow, updateGroupSchedule, updateGroupSpoilerAutoDelete, updateGroupSpoilerEnabled, updateGroupLinuxdoEnabled, } from "../db";
...
export async function handleTelegramWebhook(request, env, _ctx) {
...
async function handleCommand(command, message, env) {
...
async function sendStartMessage(env, chatId, isOwner) {
    if (!isOwner) {
        await sendMessage(env, chatId, "👋 你好！我是消息总结机器人。\n\n使用 /help 查看可用命令。");
        return;
    }
    const text = "👋 你好！我是消息总结机器人。\n\n" +
...
async function sendHelpMessage(env, chatId, isOwner) {
...
async function handleStatus(chatId, env, isOwner) {
    if (!isOwner) {
    ...
    for (const group of groups) {
        ...
        lines.push(`${statusEmoji} ${name}`, `ID: ${group.group_id}`, `定时: ${group.schedule || DEFAULT_SCHEDULE}`, `排行榜: ${leaderboardEmoji} ${group.leaderboard_schedule || DEFAULT_LEADERBOARD_SCHEDULE}`, `统计窗口: ${group.leaderboard_window || DEFAULT_LEADERBOARD_WINDOW}`, `上次总结: ${lastSummary}`, `上次排行榜: ${group.last_leaderboard_time || "无"}`, "");
    }
...
async function processCallbackData(data, chatId, userId, env, messageId) {
    ...
    if (namespace === "sch") {
        ...
        if (action === "custom") {
            ...
            await sendMessage(env, chatId, "✍️ 请输入定时表达式（支持 30m / 2h / 1d 或 5 段 Cron）。\n发送“取消”可退出。");
            return true;
        }
        return false;
    }
    ...
    if (namespace === "lb") {
        ...
        if (action === "custom") {
            ...
            await sendMessage(env, chatId, "✍️ 请输入排行榜定时表达式（支持 30m / 2h / 1d 或 5 段 Cron）。\n发送“取消”可退出。");
            return true;
        }
        if (action === "window_custom") {
            await setAdminAction(env, userId, "set_leaderboard_window", groupId, ADMIN_ACTION_TTL_MINUTES);
            await sendMessage(env, chatId, "✍️ 请输入排行榜统计窗口（例如 30m / 2h / 1d）。\n发送“取消”可退出。");
            return true;
        }
        return false;
    }
    if (namespace === "ldt") {
        ...
        if (action === "set") {
            ...
            await sendMessage(env, chatId, "✍️ 请输入全局 Linux.do Token（_t cookie 值）。\n发送 \"取消\" 可退出。\n\n" +
                "获取方式:\n" +
                "1. 登录 linux.do\n" +
                "2. 按 F12 打开开发者工具\n" +
    ...
    if (namespace === "geo") {
        ...
        if (action === "custom") {
            ...
            await sendMessage(env, chatId, "✍️ 请输入 scrape.do 地区代码（如 us、jp、sg、hk 等）。\n发送 \"取消\" 可退出。");
            return true;
        }
        if (action === "delete") {
            await deleteScrapeGeoCode(env);
...
function parseGroupIdArg(args, env, chatId) {
    if (!args.length) {
...
function isPanelTrigger(text) {
    if (!text)
        return false;
    const trimmed = text.trim();
    return trimmed === "管理面板" || trimmed === "管理";
}
async function handlePendingAdminAction(pending, message, env) {
...
async function sendGroupList(env, chatId, messageId = null) {
...
async function sendGroupActions(env, chatId, groupId, messageId = null) {
...
async function sendSummarySettingsMenu(env, chatId, groupId, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function sendLeaderboardSettingsMenu(env, chatId, groupId, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function sendScheduleMenu(env, chatId, groupId, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function sendLeaderboardMenu(env, chatId, groupId, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function sendLeaderboardWindowMenu(env, chatId, groupId, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function sendSpoilerMenu(env, chatId, groupId, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function toggleSpoilerEnabled(env, chatId, groupId, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function toggleSpoilerAutoDelete(env, chatId, groupId, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function sendLinuxdoMenu(env, chatId, groupId, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function toggleLinuxdoEnabled(env, chatId, groupId, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function sendLinuxdoTokenMenu(env, chatId, messageId = null) {
...
async function sendScrapeGeoMenu(env, chatId, messageId = null) {
...
async function sendScrapeSuperMenu(env, chatId, messageId = null) {
...
async function toggleLeaderboardEnabled(env, chatId, groupId, messageId = null, returnTo = "group") {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
        await sendPanelMessage(env, chatId, "❌ 群组未配置或暂无消息记录", messageId);
        return;
...
async function setGroupEnabled(env, chatId, groupId, enabled, messageId = null) {
    const config = await getGroupConfig(env, groupId);
    if (!config) {
    ...
    if (!messageId) {
        await sendMessage(env, chatId, enabled
            ? `✅ 已启用群组 ${groupId} 的消息总结功能`
            : `✅ 已禁用群组 ${groupId} 的消息总结功能`);
    }
}
async function applySchedule(env, chatId, groupId, schedule, messageId = null) {
    const trimmed = schedule.trim();
    if (!parseSchedule(trimmed)) {
        await sendPanelMessage(env, chatId, "❌ 无效的定时表达式，请重新输入或发送“取消”。", messageId);
        return false;
    }
    const config = await getGroupConfig(env, groupId);
    if (!config) {
...
async function applyLeaderboardSchedule(env, chatId, groupId, schedule, messageId = null) {
    const trimmed = schedule.trim();
    if (!parseSchedule(trimmed)) {
        await sendPanelMessage(env, chatId, "❌ 无效的排行榜定时表达式，请重新输入或发送“取消”。", messageId);
        return false;
    }
    const config = await getGroupConfig(env, groupId);
    if (!config) {
...
async function applyLeaderboardWindow(env, chatId, groupId, windowText, messageId = null) {
    const trimmed = windowText.trim();
    if (!parseDuration(trimmed)) {
        await sendPanelMessage(env, chatId, "❌ 无效的统计窗口，请使用 30m / 2h / 1d 这类格式。", messageId);
        return false;
    }
    const config = await getGroupConfig(env, groupId);
    if (!config) {
...
async function sendPanelMessage(env, chatId, text, messageId, options = {}) {
    if (messageId) {
        try {
...
async function runSummaryForGroupAndNotify(env, chatId, groupId) {
...
async function runLeaderboardForGroupAndNotify(env, chatId, groupId) {
...
async function handleSyncGroups(chatId, env, messageId = null) {
    await openKvSyncWindow(env, KV_SYNC_WINDOW_MS);
    const result = await syncGroupsFromRegistry(env);
    if (result.unavailable) {
        await sendPanelMessage(env, chatId, "⚠️ 未配置 GROUPS_KV，无法同步群组。", messageId);
        return;
    }
    const windowSeconds = Math.round(KV_SYNC_WINDOW_MS / 1000);
    await sendPanelMessage(env, chatId, `✅ 已同步群组：总计 ${result.total}，新增 ${result.inserted}，更新 ${result.updated}，跳过 ${result.skipped}\n⏳ KV 读写窗口已开启 ${windowSeconds} 秒`, messageId);
}
