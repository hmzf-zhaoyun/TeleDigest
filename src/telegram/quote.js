...
import { getUserAvatarFileId, getChatAvatarFileId, downloadTelegramFile, sendSticker, sendPhoto, sendMessage, telegramApi, } from "./api";
/* ── 字体 ── */
const FONT_URL = "https://cdn.jsdelivr.net/fontsource/fonts/noto-sans-sc@latest/chinese-simplified-500-normal.ttf";
let fontCache = null;
...
function segmentLine(line, emojiMap) {
    const segments = [];
    let lastIdx = 0;
...
/** 计算单个 segment 的像素宽度 */
function segmentWidth(seg, fontSize) {
    if (seg.type === "emoji")
        return fontSize * 1.2;
...
/* ── 头像获取 ── */
async function fetchAvatarBase64(env, user) {
    if (!user)
        return null;
    try {
...
async function fetchChatAvatarBase64(env, chatId) {
    try {
...
/* ── SVG 行渲染（支持 Emoji 内联） ── */
function renderLineSegments(line, emojiMap, baseX, baselineY, fontSize, ff, fill) {
    const segs = segmentLine(line, emojiMap);
    const parts = [];
    let x = baseX;
    for (const seg of segs) {
        if (seg.type === "text") {
            parts.push(`<text x="${x}" y="${baselineY}" font-size="${fontSize}" font-weight="500" fill="${fill}" font-family="${ff}">${escapeXml(seg.content)}</text>`);
            x += measureText(seg.content, fontSize);
        }
        else {
            const size = fontSize * 1.2;
            const imgY = baselineY - fontSize * 0.85;
            parts.push(`<image x="${x}" y="${imgY}" width="${size}" height="${size}" href="${seg.dataUri}"/>`);
            x += size;
        }
    }
    return parts.join("\n    ");
...
/* ── SVG 渲染 ── */
function renderMessageSvg(senderName, text, time, isPartial, avatarBase64, emojiMap, withBackground = false) {
    const layoutMax = CARD_MAX_WIDTH - PADDING * 2 - AVATAR_SIZE - GAP_AVATAR_BUBBLE - BUBBLE_PAD_X * 2;
    const maxBubbleContent = Math.min(layoutMax, MAX_BUBBLE_TEXT_WIDTH);
    const lines = wrapText(text, maxBubbleContent);
    const maxLineW = Math.max(...lines.map((l) => measureText(l, FONT_SIZE_TEXT)), measureText(senderName, FONT_SIZE_NAME) + 8, measureText(time, FONT_SIZE_TIME) + 16);
    ...
    const bubbleW = Math.min(maxBubbleContent + BUBBLE_PAD_X * 2, Math.max(maxLineW + BUBBLE_PAD_X * 2, 120));
...
/* ── 入口 ── */
export async function handleQuoteCommand(message, env) {
